...
class VideoViewerGrayscale(QWidget):
    ...
    def paintEvent(self, event):
        ...
        cur_image = self.hs_image[self.selected_channel, :, :]
        image_cp = np.require(cur_image, np.uint8, 'C')
        bytes_per_line = image_cp.shape[1]
        qp_image = QImage(image_cp, image_cp.shape[1], image_cp.shape[0], bytes_per_line, QImage.Format_Grayscale8)
    ...
    def set_data(self, hs_image):
        self.hs_image = hs_image
        self.image_width = self.hs_image.shape[2]
        self.image_height = self.hs_image.shape[1]
